...
const form = useForm({
  ...
  permissions: [...page.props.rolePermissions],
});
...
function submit() {
  form.put(`/admin/roles/${page.props.role.id}`, {
    preserveScroll: true,
  });
}
...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'Edit', href: `/admin/roles/${page.props.role.id}/edit` },
];
</script>
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <template #header>
      <h1 class="text-xl font-semibold">Edit Role: {{ page.props.role.name }}</h1>
    </template>
