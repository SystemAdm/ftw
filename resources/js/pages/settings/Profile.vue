...
import { show as showPublicProfile } from '@/routes/profile';
import { billing as billingIndex, profile as profileRoute } from '@/routes/settings';
import { update as appearanceRoute } from '@/routes/settings/appearance';
import { update as avatarRoute } from '@/routes/settings/avatar';
import { add as addGuardianRoute, remove as removeGuardianRoute } from '@/routes/settings/guardians';
import { update as headerRoute } from '@/routes/settings/header-image';
import { remove as removeMinorRoute, verify as verifyMinorRoute } from '@/routes/settings/minors';
import { update as passwordRoute } from '@/routes/settings/password';
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
                        ...
                        <Button type="submit" :disabled="appearanceForm.processing">
                            {{
                                appearanceForm.processing
                                    ? trans('pages.settings.profile.actions.saving')
                                    : trans('pages.settings.profile.actions.save')
                            }}
                        </Button>
                    </form>
                </Card>
                        ...
                        <Button type="submit" :disabled="passwordForm.processing">
                            {{
                                passwordForm.processing
                                    ? trans('pages.settings.profile.password.updating')
                                    : trans('pages.settings.profile.password.update_button')
                            }}
                        </Button>
                    </form>
                </Card>
                        ...
                        <div class="space-y-4 pt-2">
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center space-x-2">
                                    ...
                                    />
                                    <Label for="name_public" class="text-sm font-normal">{{
                                        trans('pages.settings.profile.visibility.show_full_name')
                                    }}</Label>
                                </div>
                                <div class="flex items-center space-x-2">
                                    <Checkbox
                                    ...
                                    />
                                    <Label for="email_public" class="text-sm font-normal">{{
                                        trans('pages.settings.profile.visibility.show_email')
                                    }}</Label>
                                </div>
                                <div class="flex items-center space-x-2">
                                    <Checkbox
                                    ...
                                    />
                                    <Label for="phone_public" class="text-sm font-normal">{{
                                        trans('pages.settings.profile.visibility.show_phone')
                                    }}</Label>
                                </div>
                                <div class="space-y-2">
                                    <Label for="birthday_visibility" class="text-sm font-normal">{{
                                        trans('pages.settings.profile.visibility.birthday_visibility')
                                    }}</Label>
                                ...
                                <div class="space-y-2">
                                    <Label for="postal_code_visibility" class="text-sm font-normal">{{
                                        trans('pages.settings.profile.visibility.postal_code_visibility')
                                    }}</Label>
                                    <Select v-model="profileForm.postal_code_visibility">
                                        ...
                                        <SelectContent>
                                            ...
                                            <SelectItem value="municipality">{{
                                                trans('pages.settings.profile.visibility.municipality')
                                            }}</SelectItem>
                            ...
                            <p class="text-xs text-muted-foreground">
                                <Link :href="showPublicProfile.url()" class="underline hover:text-foreground">{{
                                    trans('pages.settings.profile.view_public')
                                }}</Link>
                            </p>
                        </div>
                        ...
                        <Button type="submit" :disabled="headerForm.processing || !headerForm.header_image">
                            {{
                                headerForm.processing
                                    ? trans('pages.settings.profile.actions.uploading')
                                    : trans('pages.settings.profile.actions.upload')
                            }}
                        </Button>
                    </form>
                </Card>
                ...
                <Card class="space-y-4 p-6">
                    ...
                    <div v-if="guardians.length > 0" class="space-y-4">
                        <div v-for="guardian in guardians" :key="guardian.id" class="flex items-center justify-between rounded-lg border p-3">
                            <div>
                                <p class="font-medium">{{ guardian.name }}</p>
                                <p class="text-xs text-muted-foreground">{{ guardian.email }} ({{ guardian.relationship }})</p>
                                <div class="mt-1 flex gap-2">
                                    <span
                                        v-if="guardian.verified_guardian_at"
                                        class="rounded bg-green-100 px-1.5 py-0.5 text-[10px] text-green-700"
                                        >{{ trans('pages.settings.profile.guardians.verified_by_guardian') }}</span
                                    >
                                    <span v-else class="rounded bg-yellow-100 px-1.5 py-0.5 text-[10px] text-yellow-700">{{
                                        trans('pages.settings.profile.guardians.pending_verification')
                                    }}</span>
                                    <span v-if="guardian.verified_at" class="rounded bg-blue-100 px-1.5 py-0.5 text-[10px] text-blue-700">{{
                                        trans('pages.settings.profile.guardians.verified_by_admin')
                                    }}</span>
                                </div>
                            </div>
                    ...
                    <form @submit.prevent="addGuardian" class="space-y-4 border-t pt-4">
                ...
                <!-- Minors -->
                <Card class="space-y-4 p-6" v-if="minors.length > 0 || user.birthday">
                    <h2 class="text-lg font-semibold">{{ trans('pages.settings.profile.minors.title') }}</h2>
                    <div v-if="minors.length > 0" class="space-y-4">
                        <div v-for="minor in minors" :key="minor.id" class="flex items-center justify-between rounded-lg border p-3">
                            <div>
                                <p class="font-medium">{{ minor.name }}</p>
                                <p class="text-xs text-muted-foreground">{{ minor.email }} ({{ minor.relationship }})</p>
                                <div class="mt-1 flex gap-2">
                                    <span v-if="minor.verified_user_at" class="rounded bg-green-100 px-1.5 py-0.5 text-[10px] text-green-700">{{
                                        trans('pages.settings.profile.minors.verified_by_minor')
                                    }}</span>
                                    <span v-if="minor.verified_guardian_at" class="rounded bg-green-100 px-1.5 py-0.5 text-[10px] text-green-700">{{
                                        trans('pages.settings.profile.minors.verified_by_you')
                                    }}</span>
                                    <span v-else class="rounded bg-yellow-100 px-1.5 py-0.5 text-[10px] text-yellow-700">{{
                                        trans('pages.settings.profile.minors.verification_needed')
                                    }}</span>
                                    <span v-if="minor.verified_at" class="rounded bg-blue-100 px-1.5 py-0.5 text-[10px] text-blue-700">{{
                                        trans('pages.settings.profile.minors.verified_by_admin')
                                    }}</span>
                                </div>
                            </div>
                                ...
                                <span :class="subscription.on_grace_period ? 'text-yellow-600' : 'text-green-600'">
                                    {{
                                        subscription.on_grace_period
                                            ? trans('pages.settings.billing.cancelling')
                                            : trans('pages.settings.billing.active')
                                    }}
                                </span>
                            </p>
