...
import { checkout as checkoutRoute, portal as portalRoute } from '@/routes/settings/billing';
                ...
                <Card class="space-y-4 p-6">
                    ...
                    <div v-if="props.hasActiveSubscription" class="text-sm text-gray-500">
                        ...
                        <div v-if="formattedBillingDate" class="mt-1">
                            ...
                            <template v-else> {{ trans('pages.settings.billing.next_renewal') }}: {{ formattedBillingDate }} </template>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3">
                        <form @submit.prevent="subscribe" class="flex items-center gap-3">
                            <input class="hidden" type="text" name="price" v-model="checkoutForm.price" />
                            <Button type="submit" :disabled="checkoutForm.processing || !checkoutForm.price">
                                {{
                                    checkoutForm.processing
                                        ? trans('pages.settings.billing.redirecting')
                                        : props.hasActiveSubscription
                                          ? trans('pages.settings.billing.change_plan')
                                          : trans('pages.settings.billing.subscribe')
                                }}
                            </Button>
                        </form>
                            ...
                            <Button type="submit" variant="secondary" :disabled="portalForm.processing">
                                {{
                                    portalForm.processing
                                        ? trans('pages.settings.billing.opening')
                                        : trans('pages.settings.billing.manage_subscription')
                                }}
                            </Button>
                        </form>
                    </div>
