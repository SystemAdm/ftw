...
if (daysUntilNext === 0) daysUntilNext = 0; // If it's today, we keep it as today or should it be next week?
// The requirement says "default: first upcoming".
        ...
        <form class="max-w-3xl space-y-4" @submit.prevent="submit">
            <FieldSet>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <Field>
                        ...
                        <Select :model-value="weekdayValue" @update:model-value="(v) => (weekdayValue = v as string)">
                            ...
                            <SelectContent>
                                <SelectItem v-for="n in 7" :key="n - 1" :value="String(n - 1)">{{
                                    trans(`pages.settings.weekdays.days.${n - 1}`)
                                }}</SelectItem>
                            </SelectContent>
                        </Select>
                ...
                <div class="flex gap-2 pt-4">
                    ...
                    <Button type="button" variant="secondary" @click.prevent="router.visit(indexRoute.url())">{{
