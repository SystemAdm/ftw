...
<template>
    <SidebarLayout>
        <h1 class="mb-4 text-xl font-semibold">{{ trans('pages.settings.roles.new') }}</h1>
        ...
        <form class="max-w-xl space-y-4" @submit.prevent="submit">
            ...
            <div class="flex gap-2">
                ...
                <Button variant="secondary" @click.prevent="router.visit('/admin/roles')">{{ trans('pages.settings.locations.actions.cancel') }}</Button>
            </div>
        </form>
    </SidebarLayout>
</template>
