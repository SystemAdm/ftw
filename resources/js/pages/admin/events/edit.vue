...
import { index as indexRoute, update } from '@/routes/admin/events/index';
...
const form = useForm({
  ...
  signup_start: formatDateTime(event.signup_start),
  signup_end: formatDateTime(event.signup_end),
...
<template>
  <SidebarLayout>
    <h1 class="mb-4 text-xl font-semibold">{{ trans('pages.settings.events.edit') }}</h1>
    ...
    <form class="max-w-3xl space-y-4 pb-12" @submit.prevent="submit">
      <FieldSet>
        ...
        <div class="flex gap-2 pt-4">
          <Button type="submit" :disabled="form.processing">{{ trans('pages.settings.events.actions.save') }}</Button>
