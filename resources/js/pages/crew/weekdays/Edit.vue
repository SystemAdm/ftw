...
import { index as indexRoute, update, destroy as destroyRoute } from '@/routes/crew/weekdays';
...
const weekday = computed(() => (page.props as any).weekday);
...
const form = reactive({
  name: weekday.value.name as string,
  description: weekday.value.description as string,
  weekday: weekday.value.weekday as number,
  week_type: (weekday.value.week_type ?? 'all') as string,
  month_occurrence: (weekday.value.month_occurrence ?? 'all') as string,
  team_id: weekday.value.team_id as number | undefined,
  location_id: weekday.value.location_id as number | undefined,
  active: weekday.value.active as boolean,
  start_time: weekday.value.start_time as string,
  end_time: weekday.value.end_time as string,
  event_start: weekday.value.event_start as string | null,
  event_end: weekday.value.event_end as string | null,
});
...
const sortedExclusions = computed(() => (weekday.value.exclusions ?? []).slice().sort((a: any, b: any) => (a.excluded_date < b.excluded_date ? 1 : -1)));
...
function submit() {
  ...
  router.post(update.url(weekday.value.id), payload, {
...
const addExclusion = reactive({ url: (id: number) => `/crew/weekdays/${id}/exclusions` });
const removeExclusion = reactive({ url: (weekdayId: number, exclusionId: number) => `/crew/weekdays/${weekdayId}/exclusions/${exclusionId}` });

function addExcl() {
  if (!exclusionDate.date) return;
  router.post(addExclusion.url(weekday.value.id), { excluded_date: exclusionDate.date }, {
    onSuccess: () => {
      exclusionDate.date = '';
    }
  });
    ...
    <form class="max-w-3xl space-y-4" @submit.prevent="submit">
      <FieldSet>
        ...
        <div class="flex gap-2 pt-4">
          ...
          <Button type="button" variant="destructive" @click.stop="deleteDialogOpen = true">
