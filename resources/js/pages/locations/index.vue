    ...
    <div class="space-y-4">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <Card v-for="loc in locations.data" :key="loc.id" class="p-4">
          ...
          <div class="mt-3">
            <Link :href="`/locations/${loc.id}`" class="text-primary hover:underline">View details</Link>
          </div>
        </Card>
      </div>

      <div class="flex items-center justify-between" v-if="locations.total > locations.per_page">
        <Link v-if="locations.prev_page_url" :href="locations.prev_page_url" class="text-primary hover:underline">Previous</Link>
        <div class="text-sm text-muted-foreground">Page {{ locations.current_page }} of {{ locations.last_page }}</div>
        <Link v-if="locations.next_page_url" :href="locations.next_page_url" class="text-primary hover:underline">Next</Link>
      </div>
    </div>
  </SidebarLayout>
</template>
